...
const MTRNextTrain: React.FC<Props> = ({ theme }) => {
	...
	useEffect(() => {
		const intervalId = setInterval(handleRefreshButton, 10000);
	...
	return (
		<>
			...
			<Row className="justify-content-md-center">
				<Col>
					<Button color={ theme === 'dark'? theme : 'info'} onClick={() => handleRefreshButton()} size="lg" block>Refresh</Button>
				</Col>
			</Row>
			...
			<Row><Col><Alert color={theme === 'dark' ? theme : 'info'}>
				Current Time : <Moment format='yyyy-MM-DD HH:mm:ss' interval={1000} onChange={() => setCurrentTime(new Date())} tz="Asia/Hong_Kong">
                {currentTime}
            </Moment> HKT
			</Alert></Col></Row>
			<Table className="mt-4" dark={ theme === 'dark'}>
				<thead>
